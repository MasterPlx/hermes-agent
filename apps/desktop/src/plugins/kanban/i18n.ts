...
const ptBR: KanbanMessages = {
  ...
  col: {
    triage: { label: 'Triagem', help: 'Ideias iniciais — um agente detalha a especificação.' },
    todo: { label: 'A fazer', help: 'Aguardando dependências ou sem responsável.' },
    scheduled: { label: 'Agendada', help: 'Aguardando o horário agendado.' },
    ready: { label: 'Pronta', help: 'Dependências atendidas — atribua um perfil para o dispatcher executar a tarefa.' },
    running: {
      label: 'Em execução',
      help: 'Assumida por um worker — um agente está trabalhando nela. Definido pelo dispatcher.'
    },
    blocked: { label: 'Bloqueada', help: 'O worker solicitou intervenção humana.' },
    review: { label: 'Revisão', help: 'Um agente de revisão está verificando o trabalho. Definido pelo dispatcher.' },
    done: { label: 'Concluída', help: 'Concluída; as tarefas dependentes ficam prontas.' },
    archived: { label: 'Arquivada', help: 'Oculta na visualização padrão do quadro.' }
  },
  locked: {
    review: 'O dispatcher move a tarefa para Revisão quando um agente de revisão assume o cartão.',
    running: 'O dispatcher move a tarefa para Em execução quando um worker assume o cartão.',
    scheduled: 'Uma tarefa agendada precisa de um horário de ativação — os agentes o definem; não é possível arrastá-la para esta coluna.'
  },
  arcRunning: 'Um agente está trabalhando nisso agora.',
  arcStale: 'Tarefa assumida, mas sem sinal do worker há mais de 2 minutos — o dispatcher vai recuperá-la.',
  title: 'Kanban',
  ...
  autoAssignTip: name => `Atribuição automática a “${name}” (kanban.default_assignee) no próximo ciclo do dispatcher.`,
  ...
  groupRunning: 'Agrupar tarefas em execução por perfil',
  nSelected: n => `${n} selecionada${n === 1 ? '' : 's'}`,
  ...
  bulkFailed: (failed, total, err) =>
    `${failed} de ${total} falharam — ${err}. Os cartões com falha continuam selecionados.`,
  titlePlaceholderTriage: 'Ideia inicial — um agente vai detalhá-la',
  ...
  modelHint: 'Executa esta tarefa com um modelo e nível de raciocínio específicos. Se não for definido, usa os do perfil atribuído.',
  workspaceInherit: 'Herda a pasta do projeto do quadro',
  workspaceInheritDir: dir => `Deixe em branco para herdar ${dir}`,
  workspaceInheritGeneric: 'Deixe em branco para herdar a pasta do projeto do quadro.',
  assignee: 'Responsável',
  defaultOption: name => `${name} (padrão)`,
  parkedOption: 'sem responsável (parada — não será executada)',
  skills: 'Habilidades (separadas por vírgulas)',
  skillsPlaceholder: 'tradução, github',
  parent: 'Tarefa principal (bloqueia até ser concluída)',
  noParent: '— sem tarefa principal —',
  goalMode: 'Modo de objetivo (o worker repete o trabalho até um avaliador considerá-lo concluído)',
  creating: 'Criando...',
  ...
  estimateTip: 'Estimativa aproximada de tokens e complexidade pelo modelo auxiliar — faz uma chamada de modelo.',
  estimateTipLong:
  ...
  evtMovedTo: col => `movida para ${col}`,
  evtParentReopened: parent => `tarefa principal ${parent} reaberta`,
  ...
  evtClaimedWorker: 'assumida por um worker',
  evtWorkerStarted: 'worker iniciado',
  evtCompleted: 'concluída',
  evtBlocked: 'bloqueada — precisa de intervenção humana',
  evtUnblocked: col => `desbloqueado${col ? ` → ${col}` : ' → Pronto'}`,
  evtReclaimed: 'recuperada — devolvida à fila',
  evtSpecified: 'especificação escrita pelo agente de triagem',
  evtPromoted: 'dependências concluídas — movida para Pronta',
  evtScheduled: 'agendada para depois',
  ...
  metaCreated: 'Criado',
  metaWorkerPid: 'PID do worker',
  readyUnassignedTitle: 'Pronta, mas sem responsável — este cartão nunca será executado.',
  readyUnassignedBody:
    'O dispatcher só assume cartões em Pronta que tenham um responsável. Escolha um perfil no campo Responsável acima (ou defina um responsável padrão nas configurações de orquestração) para executá-lo em até um minuto.',
  ...
  commentsHelpRunning:
    'Esta tarefa está em execução. Sua observação será incluída no turno atual do worker em poucos segundos — sem precisar bloquear e desbloquear. “Recolocar na fila com observação” reinicia a tarefa do zero com sua observação no contexto.',
  commentsHelp:
    'Os comentários são adicionados ao histórico da tarefa. Quando um worker assume a tarefa, ele os lê como parte do contexto.',
  send: 'Enviar',
  comment: 'Comentário',
  messageWorker: 'Enviar mensagem ao worker em execução...',
  addComment: 'Adicionar um comentário...',
  deliveredLive: 'Entregue ao worker em execução em poucos segundos.',
  requeueWithNote: 'Recolocar na fila com observação',
  notePosted: 'Observação publicada — worker recolocado na fila',
  activity: n => `Atividade · ${n}`,
  runs: n => `Execuções · ${n}`,
  workerLog: 'Log do worker',
  workerLogTail: 'Log do worker · final',
  ...
  slug: slug => `slug: ${slug}`,
  project: 'Projeto',
  noProject: 'Nenhum projeto (ambientes temporários)',
  projectHintPre:
  ...
  profileDescriptionsHint: 'As descrições orientam o roteamento do decompositor. Gere-as com o modelo auxiliar ou escreva as suas.',
  profileGoodAt: 'No que este perfil é bom?',
  auto: 'Automático',
  notify: {
    completedTitle: 'Tarefa concluída',
    blockedTitle: 'Tarefa bloqueada — precisa da sua intervenção',
    blockLoopTitle: 'Tarefa encaminhada para triagem — precisa de uma decisão',
    gaveUpTitle: 'A tarefa desistiu após as tentativas',
    crashedTitle: 'O worker falhou — nova tentativa será feita',
    timedOutTitle: 'A tarefa excedeu o tempo limite — nova tentativa será feita',
